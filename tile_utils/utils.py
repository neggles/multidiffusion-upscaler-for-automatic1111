...
from typing import DefaultDict, Dict, List, Union, Tuple
...
from tile_utils.typing import Cond, ExtraNetworkData, Uncond

BBoxSettings = namedtuple(
    "BBoxSettings",
...
def build_bbox_settings(bbox_control_states) -> Dict[int, BBoxSettings]:
    ...
    for index, i in enumerate(range(0, len(bbox_control_states), NUM_BBOX_PARAMS)):
        ...
        # sanity check
        if not setting.enable or setting.x > 1.0 or setting.y > 1.0 or setting.w <= 0.0 or setting.h <= 0.0:
            continue
...
class CustomBBox(BBox):
    ...
    def __init__(
        ...
        self.feather_mask = (
            feather_mask(self.w, self.h, self.feather_ratio)
...
def split_bboxes(
    w: int, h: int, tile_w: int, tile_h: int, overlap: int = 16, init_weight: Union[Tensor, float] = 1.0
) -> Tuple[List[BBox], Tensor]:
    cols = math.ceil((w - overlap) / (tile_w - overlap))
    rows = math.ceil((h - overlap) / (tile_h - overlap))
...
def gaussian_weights(tile_w: int, tile_h: int) -> Tensor:
    ...
    def f(x, midpoint, var=0.01):
        return np.exp(-(x - midpoint) * (x - midpoint) / (tile_w * tile_w) / (2 * var)) / np.sqrt(
            2 * np.pi * var
        )

    x_probs = [
        f(x, (tile_w - 1) / 2) for x in range(tile_w)
