...
from modules import devices, images, processing, scripts, sd_samplers, shared
...
class Script(scripts.Script):
        ...
        with gr.Accordion("Tiled Diffusion", open=False):
            ...
            with gr.Row(variant="compact"):
                method = gr.Dropdown(
                    ...
                    value=Method.MULTI_DIFF.value,
                    elem_id=self.elem_id("method"),
                )
                control_tensor_cpu = gr.Checkbox(
                    label="Move ControlNet images to CPU (if applicable)",
                    value=False,
                    elem_id=self.elem_id("control_tensor_cpu"),
                )
                reset_status = gr.Button(
                    value="Free GPU", variant="tool", elem_id=self.elem_id("reset_status")
                )
            ...
            with gr.Accordion("Noise Inversion", open=True, visible=is_img2img):
                with gr.Row(variant="compact"):
                    noise_inverse = gr.Checkbox(
                        label="Enable Noise Inversion", value=False, elem_id=self.elem_id("noise_inverse")
                    )
                    noise_inverse_steps = gr.Slider(
                        minimum=1,
                        maximum=100,
                        step=1,
                ...
                with gr.Row(variant="compact"):
                    noise_inverse_retouch = gr.Slider(
                        minimum=1,
                        maximum=100,
                        step=0.1,
                        label="Retouch",
                    ...
                    noise_inverse_renoise_strength = gr.Slider(
                        minimum=0,
                        maximum=2,
                        step=0.01,
                    ...
                    noise_inverse_renoise_kernel = gr.Slider(
                        minimum=2,
                        maximum=512,
                        step=1,
                ...
                with gr.Accordion("Region Prompt Control", open=False):
                    with gr.Row(variant="compact"):
                        enable_bbox_control = gr.Checkbox(
                            label="Enable Control", value=False, elem_id=self.elem_id("enable_bbox_control")
                        )
                        draw_background = gr.Checkbox(
                            label="Draw full canvas background",
                            value=False,
                            elem_id=self.elem_id("draw_background"),
                        )
                        causal_layers = gr.Checkbox(
                            label="Causalize layers",
                            value=False,
                            visible=False,
                    ...
                    bbox_controls: List[gr.Component] = []  # control set for each bbox
                    with gr.Row(variant="compact"):
                        ref_image = gr.Image(
                            label="Ref image (for conviently locate regions)",
                            image_mode=None,
                            elem_id=f"MD-bbox-ref-{tab}",
                            interactive=True,
                        )
                        if not is_img2img:
                    ...
                    with gr.Row(variant="compact"):
                        cfg_name = gr.Textbox(
                            label="Custom Config File", value="config.json", elem_id=self.elem_id("cfg_name")
                        )
                        cfg_dump = gr.Button(value="💾 Save", variant="tool", elem_id=self.elem_id("cfg_dump"))
                        cfg_load = gr.Button(
                            value="⚙️ Load", variant="tool", elem_id=self.elem_id("cfg_load")
                        )
                        ...
                        with gr.Accordion(f"Region {i+1}", open=False, elem_id=f"MD-accordion-{tab}-{i}"):
                            with gr.Row(variant="compact"):
                                ...
                                feather_ratio = gr.Slider(
                                    ...
                                    step=0.05,
                                    visible=False,
                                    elem_id=f"MD-{tab}-{i}-feather",
                                )

                                blend_mode.change(
                                    fn=lambda x: gr_show(x == BlendMode.FOREGROUND.value),
                                    inputs=blend_mode,
                                    outputs=feather_ratio,
                                )
                            ...
                            prompt = gr.Text(
                                show_label=False,
                                placeholder=f"Prompt, will append to your {tab} prompt",
                                max_lines=2,
                                elem_id=f"MD-{tab}-{i}-prompt",
                            )
                            neg_prompt = gr.Text(
                                show_label=False,
                                placeholder="Negative Prompt, will also be appended",
                                max_lines=1,
                                elem_id=f"MD-{tab}-{i}-neg-prompt",
                            )
                            with gr.Row(variant="compact"):
                                seed = gr.Number(
                                    label="Seed",
                                    ...
                                    step=1,
                                    visible=True,
                                    elem_id=f"MD-{tab}-{i}-seed",
                                )
                                random_seed = gr.Button(
                                    value="🎲", variant="tool", elem_id=f"MD-{tab}-{i}-random_seed"
                                )
                                reuse_seed = gr.Button(
                                    value="♻️", variant="tool", elem_id=f"MD-{tab}-{i}-reuse_seed"
                                )
                                random_seed.click(
                    ...
                    cfg_dump.click(
                        fn=self.dump_regions,
                        inputs=[cfg_name, *bbox_controls],
                        outputs=cfg_tip,
                        show_progress=False,
                    )
                    cfg_load.click(
                        fn=self.load_regions,
                        inputs=[ref_image, cfg_name, *bbox_controls],
                        outputs=[*bbox_controls, cfg_tip],
                        show_progress=False,
                    )
    ...
    def dump_regions(self, cfg_name, *bbox_controls):
        if cfg_name is None or cfg_name == "":
            return gr.HTML.update(
                value='<span style="color:red">Config file name cannot be empty.</span>', visible=True
            )
    ...
    def load_regions(self, ref_image, cfg_name, *bbox_controls):
        ...
        if ref_image is None:
            return [gr_value(v) for v in bbox_controls] + [
                gr.HTML.update(
                    value='<span style="color:red">Please create or upload a ref image first.</span>',
                    visible=True,
                )
            ]
        if not file_path.exists():
            return [gr_value(v) for v in bbox_controls] + [
                gr.HTML.update(
                    value=f'<span style="color:red">Config {file_path} not found.</span>', visible=True
                )
            ]
        try:
        ...
        return [gr_value(v) for v in data_list] + [gr.HTML.update(value="Config loaded.", visible=True)]
    ...
    def process(
        ...
        *bbox_control_states: List[Any],
        ...
        """ sanitiy check """
        if (
            not enable_bbox_control
            and not splitable(p.width, p.height, tile_width, tile_height, overlap)
            and not (is_img2img and noise_inverse)
        ):
        ...
        if "png info":
            ...
            if enable_bbox_control:
                ...
                def create_bbox_random_tensors(
                    shape,
                    seeds,
                    subseeds=None,
                    ...
                    org_random_tensors = processing.create_random_tensors_original_md(
                        shape, seeds, subseeds, subseed_strength, seed_resize_from_h, seed_resize_from_w, p
                    )
                    height, width = shape[1], shape[2]
                    background_noise = torch.zeros_like(org_random_tensors)
                    background_noise_count = torch.zeros(
                        (1, 1, height, width), device=org_random_tensors.device
                    )
                    foreground_noise = torch.zeros_like(org_random_tensors)
                    foreground_noise_count = torch.zeros(
                        (1, 1, height, width), device=org_random_tensors.device
                    )
                    for i, v in bbox_settings.items():
                        seed = v.seed
                        ...
                        if v.blend_mode == "Background":
                            background_noise[:, :, y : y + h, x : x + w] += rand_tensor.to(
                                background_noise.device
                            )
                            background_noise_count[:, :, y : y + h, x : x + w] += 1
                        elif v.blend_mode == "Foreground":
                            foreground_noise[:, :, y : y + h, x : x + w] += rand_tensor.to(
                                foreground_noise.device
                            )
                    ...
                    # average
                    background_noise = torch.where(
                        background_noise_count > 1,
                        background_noise / background_noise_count,
                        background_noise,
                    )
                    foreground_noise = torch.where(
                        foreground_noise_count > 1,
                        foreground_noise / foreground_noise_count,
                        foreground_noise,
                    )
                    # paste two layers to original random tensor
                    org_random_tensors = torch.where(
                        background_noise_count > 0, background_noise, org_random_tensors
                    )
                    org_random_tensors = torch.where(
                        foreground_noise_count > 0, foreground_noise, org_random_tensors
                    )
        ...
        """ ControlNet hackin """
        try:
            from scripts.cldm import ControlNet  # noqa:
    ...
    def create_sampler_hijack(
        ...
        print(
            f"{method.value} hooked into {name} sampler. "
