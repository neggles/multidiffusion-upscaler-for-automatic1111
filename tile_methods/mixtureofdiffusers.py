...
from tile_utils.utils import gaussian_weights, BlendMode, grid_bbox, custom_bbox, keep_signature, Condition
...
class MixtureOfDiffusers(TiledDiffusion):
    ...
    def custom_apply_model(self, x_in, t_in, c_in, bbox_id, bbox) -> Tensor:
        if self.is_kdiff:
            return self.kdiff_custom_forward(
                x_in, t_in, c_in, bbox_id, bbox, forward_func=shared.sd_model.apply_model_original_md
            )
        else:
            ...
            def forward_func(x, c, ts, unconditional_conditioning, *args, **kwargs) -> Tensor:
                ...
                for k in c:
                    if isinstance(c[k], list):
                        c_in[k] = [
                            torch.cat([unconditional_conditioning[k][i], c[k][i]]) for i in range(len(c[k]))
                        ]
                    else:
    ...
    def apply_model_hijack(self, x_in: Tensor, t_in: Tensor, cond: CondDict, noise_inverse_step=-1):
        ...
        if self.draw_background:
            for batch_id, bboxes in enumerate(
                self.batched_bboxes
            ):  # batch_id is the `Latent tile batch size`
